...
function IncomeHistory() {
  const {  incomes = [], getIncomes, deleteIncome, updateIncome } = useGlobalContext(); // Default to an empty array
  ...
  useEffect(() => {
    getIncomes(); // Ensure incomes are fetched when component mounts
  }, [getIncomes]);

  const handleUpdate = useCallback((item) => {
    setCurrentItem(item);
    setFormData({id:item._id, title: item.title, amount: item.amount, date: item.date });
    setShowPopup(true);
  ...
  const displayedData = Array.isArray(incomes) ? incomes.slice(
        ...
        {displayedData.map((item) => (
          <li key={item._id}>
            ...
            <div className="actions">
              ...
              <button onClick={() => deleteIncome(item._id)}>Delete</button> {/* Pass the id to deleteIncome */}
            </div>
          </li>
        ))}
      </ul>
